...
@JBossLog
public class CassandraClientProvider extends TransactionalProvider<Client, CassandraClientAdapter>
    ...
    private CassandraClientAdapter createNewModel(RealmModel realm, Client entity, Runnable rollbackTask) {
        CassandraClientAdapter adapter = new CassandraClientAdapter(entity, session, realm, clientRepository) {
            @Override
    ...
    public ClientModel addClient(RealmModel realm, String id, String clientId) {
        ...
        adapter.updateClient(); // This is actualy strange contract - it should be the store code to call
        // updateClient
    ...
    public void addClientScopeToAllClients(
        ...
        clientRepository.findAllClientsWithRealmId(realmModel.getId()).forEach(client -> {
    ...
    public Stream<ClientModel> searchClientsByAttributes(
        ...
        return Stream.concat(
                ...
                .map(ClientModel.class::cast)
                .filter(c ->
                        attributes.isEmpty() || c.getAttributes().entrySet().containsAll(attributes.entrySet()))
                .skip(firstResult == null || firstResult < 0 ? 0 : firstResult)
                .limit(maxResults == null || maxResults < 0 ? Long.MAX_VALUE : maxResults);
    }
