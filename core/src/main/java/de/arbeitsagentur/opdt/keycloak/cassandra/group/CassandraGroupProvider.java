...
public class CassandraGroupProvider  implements GroupProvider {
    ...
    private Function<GroupValue, GroupModel> entityToAdapterFunc(RealmModel realm) {
        return origEntity -> origEntity == null ? null : new CassandraGroupAdapter(origEntity.getId(), session, realm, origEntity, getGroups(realm.getId()), this);
    }

    @Override
    public GroupModel createGroup(RealmModel realm, String id, String name, GroupModel toParent) {
        log.debugv("createGroup(%s, %s, %s, %s)", realm.getId(), id, name, toParent == null ? "null" : toParent.getId());

        if (groupExists(realm.getId(), name, toParent)) {
            throw new ModelDuplicateException("Group with the same name or parent id exists:" + name + " for parent id " + (toParent == null ? "null" : toParent.getId()));
        }
    ...
    public Long getGroupsCount(RealmModel realm, Boolean onlyTopGroups) {
        log.debugf("getGroupsCount: realmId=%s onlyTopGroups=%s", realm.getId(), Boolean.TRUE.equals(onlyTopGroups) ? "true" : "false");
    ...
    public Stream<GroupModel> getGroupsByRoleStream(RealmModel realm, RoleModel role, Integer firstResult, Integer maxResults) {
        log.debugf("getGroupsByRoleStream: realmId=%s roleId=%s firstResult=%d maxResults=%d", realm.getId(), role.getId(), firstResult, maxResults);
    ...
    public void moveGroup(RealmModel realm, GroupModel group, GroupModel toParent) {
        ...
        if (groupExists(realm.getId(), group.getName(), toParent)) {
            throw new ModelDuplicateException("Group with the same name or parent id exists:" + group.getName() + " for parent id " + (toParent == null ? "null" : toParent.getId()));
        }
    ...
    public Stream<GroupModel> searchGroupsByAttributes(RealmModel realm, Map<String, String> attributes, Integer firstResult, Integer maxResults) {
        log.debugf("searchGroupsByAttributes realmId=%s attributes=%s first=%d max=%d", realm.getId(), attributes, firstResult, maxResults);
    ...
    public Stream<GroupModel> searchForGroupByNameStream(RealmModel realm, String search, Boolean exact, Integer firstResult, Integer maxResults) {
        log.debugf("searchForGroupByNameStream: realmId=%s search=%s exact=%s first=%d max=%d", realm.getId(), search, Boolean.TRUE.equals(exact) ? "true" : "false", firstResult, maxResults);
        ...
        if (Boolean.TRUE.equals(exact)) {
            ...
            groupValueStream = groupValueStream.filter(group -> group.getName().toLowerCase().contains(search.toLowerCase()));
        }
