...
public class CassandraUserSessionProvider implements UserSessionProvider {
          ...
          lua.setUpdateHandler(
              lua1 -> {
                if (lua == lua1) { // Ensure there is no conflicting user model, only the latest
                  ...
                  cassandraUserSessionAdapter.setNote(
                      SESSION_NOTE_LIGHTWEIGHT_USER, lua1.serialize());
                }
              });
        }
  ...
  public AuthenticatedClientSessionModel createClientSession(
    ...
    UserSession userSessionEntity =
        ((CassandraUserSessionAdapter) userSession).getUserSessionEntity();
  ...
  public AuthenticatedClientSessionModel getClientSession(
    ...
    CassandraUserSessionAdapter currentSession =
  ...
  public void removeUserSessions(RealmModel realm, UserModel user) {
    ...
    userSessionRepository
        .findUserSessionsByUserId(user.getId())
        .forEach(
            session -> {
              userSessionRepository.deleteUserSession(session.getId());
  ...
  public void onClientRemoved(RealmModel realm, ClientModel client) {
    List<UserSession> relevantSessions =
        userSessionRepository.findAll().stream()
  ...
  private Stream<UserSession> getOfflineUserSessionEntityStream(
    ...
    if (userSessionEntity != null) {
      ...
      return userSessionRepository
          .findUserSessionsByAttribute(CORRESPONDING_SESSION_ID, userSessionId)
          .stream();
    }
