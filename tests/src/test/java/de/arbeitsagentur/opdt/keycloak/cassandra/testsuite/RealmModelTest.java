...
public class RealmModelTest extends KeycloakModelTest {
    ...
    public void testRealmLocalizationTexts() {
        ...
        withRealm(realmId, (session, realm) -> {
            assertThat(realm.getRealmLocalizationTexts(), aMapWithSize(1));
            assertThat(realm.getRealmLocalizationTexts(),
                hasEntry(equalTo("en"), allOf(aMapWithSize(3),
                    hasEntry(equalTo("key-a"), equalTo("text-a_en")),
    ...
    public void testIdentityProviderMappers() {
        ...
        withRealm(realmId, (s, realm) -> {
            assertThat(realm.getIdentityProviderMappersByAliasStream("testIdp").collect(Collectors.toList()), hasSize(0));
    ...
    public void testClientInitialAccesses() {
        String modelId = withRealm(realmId, (s, realm) -> realm.createClientInitialAccessModel(60, 2).getId());
        ...
        withRealm(realmId, (s, realm) -> {
            List<ClientInitialAccessModel> clientInitialAccessModels = realm.getClientInitialAccesses().collect(Collectors.toList());
    ...
    public void testComponents() {
        ...
        withRealm(realmId, (s, realm) -> {
            ...
            List<RealmModel> realms = s.realms().getRealmsWithProviderTypeStream(KeyProvider.class).collect(Collectors.toList());
        ...
        withRealm(realmId, (s, realm) -> {
            ...
            assertThat(realm.getComponentsStream("testParent", KeyProvider.class.getName()).collect(Collectors.toList()), hasSize(0));
    ...
    public void testDefaultGroup() {
        ...
        withRealm(realmId, (s, realm) -> {
            ...
            assertThat(realm.getDefaultGroupsStream().map(GroupModel::getName).findFirst().orElse(null), is("myGroup"));

            realm.removeDefaultGroup(realm.getDefaultGroupsStream().collect(Collectors.toList()).get(0));
            return null;
        });
    ...
    public void testDefaultClientScope() {
        ...
        withRealm(realmId, (s, realm) -> {
            ...
            assertThat(realm.getDefaultClientScopesStream(true).map(ClientScopeModel::getName).findFirst().orElse(null), is("myClientScope"));

            realm.removeDefaultClientScope(realm.getDefaultClientScopesStream(true).collect(Collectors.toList()).get(0));
            return null;
        });
    ...
    public void testProperties() {
        ...
        withRealm(realmId, (s, realm) -> {
            assertThat(realm.getPasswordPolicy().toString(), is(PasswordPolicy.parse(s, PasswordPolicy.PASSWORD_HISTORY_ID).toString()));
