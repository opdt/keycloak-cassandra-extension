...
public class UserConsentModelTest extends KeycloakModelTest {
    ...
    public void basicConsentTest() {
        withRealm(realmId, (session, realm) -> {
            ...
            UserConsentModel johnFooConsent = session.users().getConsentByClient(realm, john.getId(), fooClient.getId());
            ...
            UserConsentModel johnBarConsent = session.users().getConsentByClient(realm, john.getId(), barClient.getId());
            ...
            UserConsentModel maryHardcodedConsent = session.users().getConsentByClient(realm, mary.getId(), hardcodedClient.getId());
    ...
    public void getAllConsentTest() {
        withRealm(realmId, (session, realm) -> {
            ...
            List<UserConsentModel> maryConsents = session.users().getConsentsStream(realm, mary.getId())
    ...
    public void deleteUserTest() {
        ...
        withRealm(realmId, (session, realm) -> {
            Assert.assertEquals(0, session.users().getConsentsStream(realm, johnUserID.get()).count());
