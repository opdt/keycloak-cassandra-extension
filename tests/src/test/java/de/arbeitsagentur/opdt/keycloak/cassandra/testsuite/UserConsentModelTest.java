...
public class UserConsentModelTest extends KeycloakModelTest {
  ...
  private boolean isClientScopeGranted(
      RealmModel realm, String scopeName, UserConsentModel consentModel) {
    ...
    withRealm(
        ...
        (session, realm) -> {
          ...
          Assert.assertNotNull(
              "Last Updated Date should be set", johnFooConsent.getLastUpdatedDate());
          ...
          Assert.assertNotNull(
              "Last Updated Date should be set", johnBarConsent.getLastUpdatedDate());

          UserConsentModel maryConsent =
              session.users().getConsentByClient(realm, mary.getId(), fooClient.getId());
          ...
          ClientModel hardcodedClient =
              session.clients().getClientByClientId(realm, "hardcoded-client");
          ...
          Assert.assertNotNull(
              "Last Updated Date should be set", maryHardcodedConsent.getLastUpdatedDate());

          Assert.assertNull(
              session.users().getConsentByClient(realm, john.getId(), hardcodedClient.getId()));

          Assert.assertNull(
              session.users().getConsentByClient(realm, mary.getId(), barClient.getId()));
  ...
  public void getAllConsentTest() {
    withRealm(
        ...
        (session, realm) -> {
          ...
          Assert.assertEquals(2, session.users().getConsentsStream(realm, john.getId()).count());
          ...
          Assert.assertEquals(hardcodedClient.getId(), maryHardcodedConsent.getClient().getId());
          Assert.assertEquals(0, maryHardcodedConsent.getGrantedClientScopes().size());
  ...
  public void deleteUserTest() {
    ...
    withRealm(
        ...
        (session, realm) -> {
          Assert.assertEquals(
              0, session.users().getConsentsStream(realm, johnUserID.get()).count());

          UserModel mary = session.users().getUserByUsername(realm, "mary");
          Assert.assertEquals(2, session.users().getConsentsStream(realm, mary.getId()).count());
  ...
  public void deleteClientTest() {
    ...
    withRealm(
        ...
        (session, realm) -> {
          ...
          Assert.assertNull(
              session.users().getConsentByClient(realm, john.getId(), barClientID.get()));
