...
public class ExportImportManagerTest extends KeycloakModelTest {
  ...
  public void createEnvironment(KeycloakSession s) {
    ...
    client.setAuthenticationFlowBindingOverride(
        AuthenticationFlowBindings.BROWSER_BINDING, browser.getId());
    ...
    s.users()
        .addFederatedIdentity(
            realm, user, new FederatedIdentityModel("idpId", "idpUserId", "idpUserName"));
  }
  ...
  public void testExportImport() {
    ...
    inComittedTransaction(
        session -> {
          ...
          try {
            rep =
                JsonSerialization.readValue(outputStream.toByteArray(), RealmRepresentation.class);
          } catch (IOException e) {
          ...
          RoleModel newlySavedDefaultRole =
              session.roles().addRealmRole(newRealm, defaultRole.getName());
    ...
    inComittedTransaction(
        session -> {
          ...
          assertThat(
              importedRealm.getSsoSessionMaxLifespan(),
              is(originalRealm.getSsoSessionMaxLifespan()));
          assertThat(
              importedRealm.getSsoSessionIdleTimeout(),
              is(originalRealm.getSsoSessionIdleTimeout()));
          assertThat(
              importedRealm.getClientSessionMaxLifespan(),
              is(originalRealm.getClientSessionMaxLifespan()));
          assertThat(
              importedRealm.getAccessCodeLifespan(), is(originalRealm.getAccessCodeLifespan()));
          assertThat(
              importedRealm.getAccessCodeLifespanLogin(),
              is(originalRealm.getAccessCodeLifespanLogin()));
          assertThat(
              importedRealm.getAccessTokenLifespan(), is(originalRealm.getAccessTokenLifespan()));
          assertThat(
          ...
          assertThat(
              importedRealm.isUserManagedAccessAllowed(),
              is(originalRealm.isUserManagedAccessAllowed()));
          assertThat(
          ...
          assertThat(
              importedRealm.getAttribute("testAttribute"),
              is(originalRealm.getAttribute("testAttribute")));
          ...
          ClientModel importedClient =
              session.clients().getClientByClientId(importedRealm, "client1");
          assertThat(importedClient.getClientScopes(true).values(), hasSize(1));
          assertThat(
              importedClient.getClientScopes(true).values().iterator().next().getName(),
              is("scope1"));
          assertThat(importedClient.getClientScopes(false).values(), hasSize(1));
          assertThat(
              importedClient.getClientScopes(false).values().iterator().next().getName(),
              is("scope2"));
          assertThat(
              importedClient.getScopeMappingsStream().collect(Collectors.toList()), hasSize(3));
          assertThat(
              importedClient
                  ...
                  .collect(Collectors.toList()),
              containsInAnyOrder(
                  "scopeMappingRole", "scopeMappingRoleClient", "scopeMappingRoleClient2"));
          ...
          assertThat(
              ...
              containsInAnyOrder(
                  "clientRole1",
                  "clientRole2",
                  "scopeMappingRoleClient",
                  "scopeMappingRoleClient2"));
          ...
          assertThat(
              ...
              containsInAnyOrder(
                  "realmRole1", "realmRole2", "scopeMappingRole", "compositeRealmRole"));
          List<RoleModel> compositeRoles =
              realmRoles.stream()
          ...
          assertThat(
              subflow2Execution.getRequirement(),
              is(AuthenticationExecutionModel.Requirement.ALTERNATIVE));

          assertThat(subflow2.getAlias(), is("subflow-2"));
          assertThat(subflow2Execution.getFlowId(), is(subflow2.getId()));

          List<AuthenticationExecutionModel> subflow2ChildExecutions =
              importedRealm
          ...
          assertThat(
              subflow2ChildExecutions.stream()
                  ...
                  .collect(Collectors.toList()),
              containsInAnyOrder(
                  "push-button-authenticator", UsernamePasswordFormFactory.PROVIDER_ID));
          assertThat(
              subflow2ChildExecutions.stream()
                  .allMatch(
                      e ->
          ...
          ClientModel testAppFlowClient =
              session.clients().getClientByClientId(importedRealm, "test-app-flow");
          assertThat(
              testAppFlowClient.getBaseUrl(),
              is("http://localhost:8180/auth/realms/master/app/auth"));
          assertThat(
              testAppFlowClient.getManagementUrl(),
              is("http://localhost:8180/auth/realms/master/app/admin"));
          ...
          assertThat(
              ...
              is("http://localhost:8180/auth/realms/master/app/auth/*"));
          assertThat(
              testAppFlowClient.getAuthenticationFlowBindingOverrides().values(), hasSize(1));
          assertThat(
              testAppFlowClient.getAuthenticationFlowBindingOverride(
                  AuthenticationFlowBindings.BROWSER_BINDING),
