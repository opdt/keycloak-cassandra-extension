...
public class ExportImportManagerTest extends KeycloakModelTest {
    ...
    public void testExportImport() {
        ...
        inComittedTransaction(session -> {
            ...
            assertThat(importedRealm.getAccessCodeLifespanUserAction(), is(originalRealm.getAccessCodeLifespanUserAction()));
            assertThat(importedRealm.getAccessTokenLifespanForImplicitFlow(), is(originalRealm.getAccessTokenLifespanForImplicitFlow()));
            assertThat(importedRealm.isUserManagedAccessAllowed(), is(originalRealm.isUserManagedAccessAllowed()));
            assertThat(importedRealm.getActionTokenGeneratedByAdminLifespan(), is(originalRealm.getActionTokenGeneratedByAdminLifespan()));
            assertThat(importedRealm.getActionTokenGeneratedByUserLifespan(), is(originalRealm.getActionTokenGeneratedByUserLifespan()));
            ...
            assertThat(importedRealm.getSupportedLocalesStream().collect(Collectors.toList()), is(originalRealm.getSupportedLocalesStream().collect(Collectors.toList())));
            ...
            assertThat(importedRealm.getSsoSessionIdleTimeoutRememberMe(), is(originalRealm.getSsoSessionIdleTimeoutRememberMe()));
            ...
            AuthenticationExecutionModel subflow2Execution = importedRealm.getAuthenticationExecutionByFlowId(subflow2.getId());
            ...
            List<AuthenticationExecutionModel> subflow2ChildExecutions = importedRealm.getAuthenticationExecutionsStream(subflow2.getId()).collect(Collectors.toList());
            assertThat(subflow2ChildExecutions, hasSize(2));
            assertThat(subflow2ChildExecutions.stream().map(AuthenticationExecutionModel::getAuthenticator).collect(Collectors.toList()),
                containsInAnyOrder("push-button-authenticator", UsernamePasswordFormFactory.PROVIDER_ID));
            assertThat(subflow2ChildExecutions.stream().allMatch(e -> e.getRequirement().equals(AuthenticationExecutionModel.Requirement.REQUIRED)), is(true));
            ...
            assertThat(testAppFlowClient.getRedirectUris().iterator().next(), is("http://localhost:8180/auth/realms/master/app/auth/*"));
            assertThat(testAppFlowClient.getAuthenticationFlowBindingOverrides().values(), hasSize(1));
            assertThat(testAppFlowClient.getAuthenticationFlowBindingOverride(AuthenticationFlowBindings.BROWSER_BINDING), is(browserFlow.getId()));
