...
public class ClientModelTest extends KeycloakModelTest {
  ...
  public void testClientsBasics() {
    ...
    ClientModel originalModel =
        withRealm(realmId, (session, realm) -> session.clients().addClient(realm, "myClientId"));
    ClientModel searchClient =
        withRealm(
            realmId,
            (session, realm) -> {
    ...
    // Find by id
    {
      ClientModel model =
          withRealm(
              realmId,
              (session, realm) -> session.clients().getClientById(realm, originalModel.getId()));
    ...
    // Find by clientId
    {
      ClientModel model =
          withRealm(
              realmId,
              (session, realm) -> session.clients().getClientByClientId(realm, "myClientId"));
      assertThat(model, notNullValue());
    ...
    // Search by clientId
    {
      ...
      withRealm(
          ...
          (session, realm) -> {
            ...
            assertThat(
                session.clients().searchClientsByClientIdStream(realm, "%", 0, 10).count(),
                is(equalTo(2L)));
            return null;
          });
    }
    ...
    // using Boolean operand
    {
      Map<ClientModel, Set<String>> allRedirectUrisOfEnabledClients =
          withRealm(
              realmId,
              (session, realm) -> session.clients().getAllRedirectUrisOfEnabledClients(realm));
      assertThat(allRedirectUrisOfEnabledClients.values(), hasSize(1));
      assertThat(
          allRedirectUrisOfEnabledClients.keySet().iterator().next().getId(),
          is(equalTo(searchClient.getId())));
    }
  ...
  public void testClientScopes() {
    ...
    withRealm(
        realmId,
        (session, realm) -> {
          List<String> actualClientScopes =
              session
                  .clientScopes()
          ...
          ClientScopeModel clientScopeById =
    ...
    withRealm(
        ...
        (session, realm) -> {
          ...
          ClientModel copyClient =
              RepresentationToModel.createClient(session, realm, representation);
  ...
  public void testClientScopesBinding() {
    ...
    withRealm(
        ...
        (session, realm) -> {
          ...
          assertThat(
              "Client Scope contains 'scope1':", clientScopes1.containsKey("scope1"), Is.is(true));
          assertThat(
              "Client Scope contains 'scope2':", clientScopes1.containsKey("scope2"), Is.is(false));
          assertThat(
              "Client Scope contains 'scope3':", clientScopes1.containsKey("scope3"), Is.is(false));

          Map<String, ClientScopeModel> clientScopes2 = client.getClientScopes(false);
          assertThat(
              "Client Scope contains 'scope1':", clientScopes2.containsKey("scope1"), Is.is(false));
          assertThat(
              "Client Scope contains 'scope2':", clientScopes2.containsKey("scope2"), Is.is(true));
          assertThat(
              "Client Scope contains 'scope3':", clientScopes2.containsKey("scope3"), Is.is(true));
    ...
    withRealm(
        ...
        (session, realm) -> {
          ...
          Map<String, ClientScopeModel> clientScopes1 = client.getClientScopes(true);
          assertThat(
              "Client Scope contains 'scope1':", clientScopes1.containsKey("scope1"), Is.is(false));
          assertThat(
              "Client Scope contains 'scope2':", clientScopes1.containsKey("scope2"), Is.is(false));
          assertThat(
              "Client Scope contains 'scope3':", clientScopes1.containsKey("scope3"), Is.is(false));
          ...
          assertThat(
              "Client Scope contains 'scope1':", clientScopes2.containsKey("scope1"), Is.is(false));
          assertThat(
              "Client Scope contains 'scope2':", clientScopes2.containsKey("scope2"), Is.is(false));
          assertThat(
              "Client Scope contains 'scope3':", clientScopes2.containsKey("scope3"), Is.is(true));
  ...
  public void testProperties() {
    ...
    withRealm(
        ...
        (s, realm) -> {
          ...
          assertThat(
              client.getAuthenticationFlowBindingOverrides().values(), containsInAnyOrder("flow1"));
  ...
  public void testClientScopesCrud() {
    ...
    withRealm(
        ...
        (s, realm) -> {
          ClientModel client = s.clients().getClientById(realm, clientId);
          Map<String, ClientScopeModel> defaultScopes =
              s.clients().getClientScopes(realm, client, true);
          Map<String, ClientScopeModel> nonDefaultScopes =
              s.clients().getClientScopes(realm, client, false);
  ...
  public void testClientSearch() {
    ...
    withRealm(
        ...
        (s, realm) -> {
          ...
          List<ClientModel> clientsByClientId =
              s.clients()
          ...
          List<ClientModel> clientsByAttribute =
              s.clients()
